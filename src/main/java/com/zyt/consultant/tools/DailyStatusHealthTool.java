...
public class DailyStatusHealthTool {
    ...
    @Tool("根据当前会话memoryId判断用户当天饮水量、睡眠时长和运动时长是否健康并给出建议，输入memoryId例如user-4或chat:memory:user:4:session:session-abc")
    public String evaluateTodayStatusByMemoryId(String memoryId) {
        ...
        ReferenceSourceContext.addSource(DAILY_STATUS_SOURCE);
